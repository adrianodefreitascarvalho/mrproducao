...
const ForehandDimensions = () => {
  ...
  const handleGeneratePdf = async (itemId: string) => {
    ...
    const templatePath = '/pdf-templates/forehand_dimensions.pdf';
    const schemaPath = '/pdf-templates/forehand_dimensions_schema.json';
    const clientName = client ? `${client.first_name}_${client.last_name}`.replace(/\s+/g, '_') : 'desconhecido';
    const outputFileName = `FolhaObra_Fuste_${clientName}_${new Date().toISOString().split('T')[0]}.pdf`;
