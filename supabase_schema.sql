...
-- Tabela para registo de interações com Prospects (Mensagens e Respostas)
...
-- Política de acesso (Permitir tudo para desenvolvimento)
DROP POLICY IF EXISTS "Acesso total a prospect_interactions" ON public.prospect_interactions;
CREATE POLICY "Acesso total a prospect_interactions"
ON public.prospect_interactions
FOR ALL
USING (true);

-- Tabela para registo de interações com Prospects
CREATE TABLE IF NOT EXISTS public.prospect_interactions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    prospect_id UUID NOT NULL REFERENCES public.prospects(id) ON DELETE CASCADE,
