...
const ProspectForm = ({ prospect, onSave, onCancel }: { prospect: Partial<Prospect>, onSave: (data: Partial<Prospect>) => void, onCancel: () => void }) => {
  ...
  const [weaponBrand, _setWeaponBrand] = useState(prospect.weapon_brand ?? '');
...
const EditProspectPage = () => {
  ...
  const { prospects, addProspect, updateProspect } = useProductionStore();

  const prospectToEdit = useMemo(() => (isNew ? {} : prospects.find((p: Prospect) => p.id === id)), [prospects, id, isNew]);

  const handleSave = async (data: Partial<Prospect>) => {
    if (isNew) {
      await addProspect(data as Prospect);
    } else {
